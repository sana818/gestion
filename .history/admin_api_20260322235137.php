...
/**
 * Valide le JWT et vérifie que l'utilisateur est RESPONSABLE RH
 * @return object Le token décodé
 */
function validateRHRole() {
    ...
    try {
        ...
        // 🔥 MODIFICATION ICI : Vérifier que le rôle est 'rh' ou 'responsable_rh'
        $allowedRoles = ['rh', 'responsable_rh', 'responsable rh', 'hr'];
        $userRole = strtolower($decoded->role ?? '');
        
        if (!in_array($userRole, $allowedRoles)) {
            sendError('Accès réservé aux responsables RH', 403);
        }
...
// 🔥 MODIFICATION ICI : Valider avec la fonction responsable RH
$decoded = validateRHRole();
...
switch ($_SERVER['REQUEST_METHOD']) {
    ...
    case 'GET':
        ...
        $poste  = $_GET['poste'] ?? '';
        $query  = "
            SELECT r.id, r.nom, r.prenom, r.email, r.role, COALESCE(e.poste, 'Non défini') as poste
            FROM registre r
            LEFT JOIN emplois e ON r.id = e.employe_id
        ";
        $params = [];
