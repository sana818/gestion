...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['action']) && $_GET['action'] === 'get_historique') {
        ...
        try {
            ...
            if ($has_table_salles) {
                // Avec jointure sur la table salles
                $sql = "SELECT 
                            h.id,
                            CONCAT(e.prenom, ' ', e.nom) AS employe_nom,
                            s.nom                         AS salle,
                            DATE(h.date_entree)           AS date_entree,
                            h.heure_entree,
                            h.heure_sortie
                        FROM historique_salles h
                        LEFT JOIN registre e ON h.employe_id = e.id
                        LEFT JOIN salles   s ON h.salle_id   = s.id
                        ORDER BY h.date_entree DESC, h.heure_entree DESC
                        LIMIT 200";
            } else {
                // Sans table salles — afficher salle_id directement
                $sql = "SELECT 
                            h.id,
                            CONCAT(e.prenom, ' ', e.nom) AS employe_nom,
                            h.salle_id                    AS salle,
