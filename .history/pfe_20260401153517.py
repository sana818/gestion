...
def tester_rfid(rfid_code):
    ...
    if user:
        ...
        # --- Vérification double scan (RFID détecté deux fois) ---
        count_query = """
            SELECT COUNT(*) FROM presence 
            WHERE employe_id = %s AND date = %s
        """
        cursor.execute(count_query, (user_id, today))
        count = cursor.fetchone()[0]

        if count == 2:
            update_statut = "UPDATE registre SET statut = 'en_attente' WHERE id = %s"
            cursor.execute(update_statut, (user_id,))
            db.commit()
            print(f"⚠️ Carte scannée 2 fois → statut remis à 'en_attente' pour {prenom} {nom}")
            return
