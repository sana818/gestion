...
// ── Validation JWT (rôle directeur uniquement) ──
...
$stmt = $db->prepare("SELECT id, nom, role FROM registre WHERE id = ?");
...
if ($action === 'nommer_rh') {
    ...
    if ($emp['role'] === 'administrateur') {
        sendError('Impossible de modifier le rôle d\'un administrateur');
    }

    try {
        $stmt = $db->prepare("UPDATE registre SET role = 'responsable_rh' WHERE id = ?");
        $stmt->execute([$employe_id]);
        sendJson([
            'success' => true,
...
if ($action === 'revoquer_rh') {
    ...
    try {
        $stmt = $db->prepare("UPDATE registre SET role = 'employe' WHERE id = ?");
        $stmt->execute([$employe_id]);
        sendJson([
            'success' => true,
