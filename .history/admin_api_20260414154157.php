...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($action === 'get_employee') {
        ...
        $stmt = $db->prepare("
            SELECT id, nom, prenom, email, role,
                   numero_telephone, date_naissance, poste, photo
            FROM employes
            WHERE id = ?
        ");
        $stmt->execute([$id]);
    ...
    elseif ($action === 'get_all_employees') {
        $stmt = $db->query("
            SELECT id, nom, prenom, email, role,
                   numero_telephone, poste, statut, photo
            FROM employes
            ORDER BY nom ASC
        ");
...
elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (isset($input['action']) && $input['action'] === 'update_photo') {

        $employee_id = intval($input['employee_id'] ?? 0);
        $photo_url = $input['photo_url'] ?? '';

        if (!$employee_id) sendError("ID employé manquant");
        if (!$photo_url) sendError("URL photo manquante");
        ...
        $stmt->execute([$photo_url, $employee_id]);
